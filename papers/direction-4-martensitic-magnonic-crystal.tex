...
\author{C.~Scott}
\affiliation{Apt Systems, Enumclaw, Washington, USA}
...
The author used Claude (Anthropic) and ChatGPT (OpenAI) for manuscript drafting assistance, editorial feedback, and non-authoritative code review. AI tools were not used to generate final scientific claims, select citations without verification, or produce unverified numerical results. All simulations, figures, citations, analysis, and conclusions were checked by the author, who takes full responsibility for the work. The author thanks the open-source magnonics community for developing and maintaining mumax3~\cite{Vansteenkiste2014}. The 2D micromagnetic simulations were performed on an NVIDIA RTX 3070 Ti GPU.
\end{acknowledgments}

\section*{Data Availability}
The transfer-matrix simulation code, disorder-analysis scripts, figure-generation scripts, mumax3 input file, FFT post-processing script, and numerical data used to generate all figures are available from the author upon reasonable request and will be deposited in a public repository (Zenodo or GitHub) upon acceptance.
